  ...
  if (__glibc_unlikely (GLRO(dl_bind_not)))
	...
	if(l->l_shared_flag == 0 && l->l_protected_flag == 1)
	{
  		...
  		//ElfW(Addr) temp = ElfW(Addr) 
  	 	...
  	 	_dl_dprintf(1, "jump_addr:%lx\n", (unsigned long)l->l_jump_addr + JUMP_SIZE * (reloc_arg + 1));
  		...
  		*temp = value;
  		...
  		return elf_machine_fixup_plt(l, result, reloc, rel_addr, l->l_jump_addr + JUMP_SIZE * (reloc_arg + 1));
	}
  //else
